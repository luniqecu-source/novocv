...
export function ImportarPanel() {
  ...
  const handle = async (file: File | undefined) => {
    ...
    try {
      ...
      notify(
        `Documento leído (${Math.round(
          confidence * 100
        )}% reconocido). Revisa los campos.`,
        'ok'
      )
    } catch (error) {
      notify(
  ...
  return (
    ...
    >
      ...
      <p className="text-[11.5px] leading-relaxed text-muted">
        La lectura es heurística: reconoce encabezados en español y
        rangos de años. Si el resultado queda desordenado, el
        asistente puede reinterpretar el mismo texto con más precisión.
      </p>
